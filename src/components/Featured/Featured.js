...
export default function Featured() {
    ...
    //     //         iDoc =
    //     //             iframe.contentWindow || // sometimes glamorous naming of variable
    ...
    //     // }
    //     if (iframes) {
    //         iframes.addEventListener('load', (e) => {
    //             debugger;
    ...
    const debouncedChangeHandler = useMemo(() => debounce(handleScroll, 5000), []);
    ...
    return (
      ...
      > */}
            ...
            {featureData.map((card, index) => {
                return <Card title={card.name} active={index === current} key={index} progress={progress} />;
            })}
        </section>
    );
}
